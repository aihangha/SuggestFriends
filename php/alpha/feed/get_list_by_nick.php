...
if (isset($_GET['xemail'])&&isset($_GET['lastupdate'])) {
    $email = $_GET['xemail'];
...
if($lastupdate==="last30"){
	$result = mysql_query("SELECT * FROM (SELECT *FROM feed ORDER BY lastupdate DESC LIMIT 30) sub ORDER BY lastupdate ASC");
} else{
	$result = mysql_query("SELECT * FROM (SELECT *FROM feed  WHERE lastupdate>'$lastupdate') sub ORDER BY lastupdate ASC");
}
//$result = mysql_query("SELECT *FROM feed");
